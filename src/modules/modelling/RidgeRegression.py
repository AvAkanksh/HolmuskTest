...
logBase = config['logging']['logBase'] + '.modules.modelling.RidgeRegression'
...
def modelBuilder(logger):
    ''' This function helps us to build the Ridge Regression model using sklearn
...
def print_coefficients(logger,model):
    ...
    '''
    print("Coefficient of the trained Ridge Regression model for the given training dataset:\n",model.coef_)
...
def plot_graphs(logger,X_train,y_train,X_test,y_test,y_pred):
    ...
    plt.xlabel("X_train")
    plt.ylabel("y_train")   
    plt.savefig("../results/X_train_VS_y_train_Ridge.png")
    plt.show()    
    plt.figure()
    ...
    plt.xlabel("X_test")
    plt.ylabel("y")   
    plt.savefig("../results/X_test_VS_y_test_&_y_pred_Ridge.png")
    plt.show()    
